                ...
                @switch($setting->type)
                    @case(Modules\Core\Enums\SettingType::IMAGE->value)
                        ...
                        <x-dashboard::forms.image-upload
                            ...
                            :preview="$this->getImagePreview($setting->key)"
                            :file-name="$this->getClientOriginalName($setting->key)"
                            :upload-key="$setting->key"
                            target="form.{{ $setting->key }}"
                    ...
                    @default
                        ...
                        <x-dashboard::forms.input :label="$setting->title"  :name="$name"
                            wire:model.defer="form.{{ $setting->key }}" target="form.{{ $setting->key }}" />
                @endswitch
            @endforeach
        </div>
