<div class="container mx-auto px-4 rtl">
    <section class="rounded-xl border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
        ...
        @if(isset($showAdminPasswordMessage) && $showAdminPasswordMessage)
            <div class="rounded-xl border border-danger-400 p-2 sm:p-3 shadow-sm mb-4">
                اطلاعات خود را از طریق این فرم ویرایش نمایید.
                <br> در صورت عضویت از طریق لینک ثبت نام
                پسورد شما شماره موبایلتان است.
            </div>
        @endif
        <form wire:submit="store">
            <div class="flex flex-col gap-4">
                @if(isset($showImagePreview) && $showImagePreview)
                    <div class="space-y-4">
                        <div>
                ...
                @else
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-900 mb-1">@lang('user::attributes.image')
                            </label>
                            <p class="text-xs text-gray-500 mb-2">
                                @lang('media::attributes.image_formats'):
                                {{config('media.validations.image.mimes')}} (@lang('media::attributes.max')
                                {{config('media.validations.image.max') / 1024}} @lang('media::attributes.MB'))
                            </p>
                        </div>
                        <div class="flex flex-col sm:flex-row gap-4">
                            <div class="w-full flex-1">
                                <label
                                    class="flex flex-col items-center justify-center w-full h-40 border rounded-3xl cursor-pointer hover:bg-gray-100 transition-colors border-[#D3E0E4] relative overflow-hidden {{isset($form['image']) ? 'border-green-500 bg-green-50' : ''}}">
                                    @if (isset($form['image']) && is_object($form['image']))
                                        <div class="w-full h-full flex items-center justify-center p-2 text-center">
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-600"> @lang('media::attributes.image_selected') </p>
                                                <p class="text-xs text-gray-500 mt-1">
                                                    @lang('media::messages.click_again_for_change_image')
                                                </p>
                                            </div>
                                    ...
                                    @else
                                        <div class="flex flex-col items-center justify-center pt-5 pb-6 px-4 text-center"> <svg
                                            ...
                                            <p class="mb-2 text-sm text-gray-500"> <span class="font-semibold">
                                                ...
                                                @lang('media::messages.or_drop_image') </p>
                                        </div>
                                    @endif
                                    <input type="file" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                                        accept="image/jpeg, image/png, image/jpg" wire:model="form.image" />
                            ...
                            @if (isset($form['image']) && is_object($form['image']))
                                <div class="w-[200px] flex-shrink-0">
                                    <div class="h-40 w-full rounded-3xl overflow-hidden border border-[#D3E0E4] relative group">
                                        ...
                                        <div class="absolute inset-0 flex items-center justify-center"> <button type="button"
                                                wire:click="removeImage"
                                                class="p-2 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity transform hover:scale-110"
                                                title="@lang('media::attributes.delete_image')"> <svg
                ...
                <div class="grid gap-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="mb-2 block text-sm font-medium">{{ __('user::attributes.mobile') }} </label>
                            <input type="text" wire:model.defer="form.mobile" class="w-full rounded-lg border-gray-300"
                                placeholder="{{__('user::messages.enter_mobile')}}" />
                            @error('form.mobile')
                                <span class="text-red-500 text-xs">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            </label> <input type="password" wire:model.defer="form.password"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="mb-2 block text-sm font-medium">{{ __('user::attributes.status') }} </label>
